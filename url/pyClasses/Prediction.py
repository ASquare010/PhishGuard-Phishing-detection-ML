from url.pyClasses.FeatureExtraction import FeatureExtractionURLS
import joblib
import pandas as pd
...
class PredictionURLS: 
        ...
        def __init__(self,urls = [],modelPath = "url/model/bestmodel.pkl"):                

                for url in urls:
                        self.resultOutput = self.resultOutput + self.predict(url,modelPath)


        def predict(self,url,modelPath = "url/model/bestmodel.pkl"):
